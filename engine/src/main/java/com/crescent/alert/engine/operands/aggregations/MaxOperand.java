...
public class MaxOperand extends AbstractAggregationOperand {
    ...
    public Object getValue(ProcessingContext context) {
        final Operand innerOpt = this.getOperand();
        Event value = filterForCalculus(filter(context.getPreviousEvents(), context.getThreshold()), context.getThreshold()).min((event, t1) -> {
            BigDecimal b1 = new BigDecimal(innerOpt.getValue(new ProcessingContext(event, Arrays.asList(event), context.getThreshold())).toString());
            BigDecimal b2 = new BigDecimal(innerOpt.getValue(new ProcessingContext(t1, Arrays.asList(t1), context.getThreshold())).toString());
            return b2.compareTo(b1);
        }).orElse(null);
        ...
        return innerOpt.getValue(new ProcessingContext(value, Arrays.asList(value), context.getThreshold()));
    }
